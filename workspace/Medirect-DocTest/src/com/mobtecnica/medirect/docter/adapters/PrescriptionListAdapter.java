...
import com.mobtecnica.medirect.docter.connection.HttpRequestHelperForMyHistory;
...
public class PrescriptionListAdapter extends BaseExpandableListAdapter {
	...
	public PrescriptionListAdapter(Context activity,
	...
	private void buildChildUI(View convertView, int childPosition) {
		// profileImage
		Picasso.with(act)
				.load(patModel.get(childPosition).getPhoto())
	...
	private void childviewClickListeners(final int groupPosition,
		...
		buttonViewPrescriptions.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				if (Utilities.getInstance(context).isNetAvailable()) {
					...
					LoadMyHistory myHistory = new LoadMyHistory(fractivity, patModel.get(groupPosition).getId());
					...
					.prescriptionDetailNew(
							fractivity,
							patModel.get(groupPosition).getId(), "", "", "0",patModel.get(groupPosition),myHistoryModel,false);
				} else {
		...
		newprescriptions.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				if (Utilities.getInstance(context).isNetAvailable()) {
				/*	Fragment frg = AddNewPrescription.newInstanceOfAddNewPrescription(patModel.get(groupPosition));
					Utilities.getInstance(context)
					.changeChildFragment(
							frg,
							"AddNewPrescription",fractivity
							);*/
					HttpRequestHelperForMyHistory.MyHistory(act,patModel.get(groupPosition).getId(), patModel.get(groupPosition));
